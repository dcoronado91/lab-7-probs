...
# Mínimo teórico sin repetidas: ceil(N/S) = ceil(100/7) = 15 sobres
...
print(f"     En simulación, ¿alguna sim usó exactamente {minimo_teorico}? "
    f"{'Sí' if (sobres_totales == minimo_teorico).any() else 'No'}")
print(f"  2. H_100 (exacto)       : {H_N:.6f}")
print(f"     H_100 ≈ ln(100)+γ    : {np.log(100)+0.5772:.6f}")
...
plt.tight_layout()
plt.savefig('/mnt/user-data/outputs/etapa1_histograma.png', dpi=150)
plt.close()
print("\n✓ Histograma guardado.")